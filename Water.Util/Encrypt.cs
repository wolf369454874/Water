...
namespace Water.Util
{
    ...
    {
        //针对正常应用加密算法
        static string CIV = "Mi9l/+7Zujhy12se6Yjy111A";  //初始化向量
        static string CKEY = "jkHuIy9D/9i="; //密钥（常量）
        ...
        /// <returns></returns>
        public static string EncryptString(string Value, string _CIV)
        {
            //return Value;
            if (string.IsNullOrEmpty(_CIV))
                _CIV = CIV;
            ...
            byte[] byt;
            using(SymmetricAlgorithm mCSP = new DESCryptoServiceProvider())
            {
                //CreateEncryptor创建(对称数据)加密对象
                ct = mCSP.CreateEncryptor(Convert.FromBase64String(CKEY), Convert.FromBase64String(_CIV)); //用指定的密钥和初始化向量创建对称数据加密标准
            }
        ...
        public static string DecryptString(string Value, string _CIV)
        {
            if (string.IsNullOrEmpty(_CIV))
                _CIV = CIV;
            ...
            using (SymmetricAlgorithm mCSP = new DESCryptoServiceProvider())
            {
                ct = mCSP.CreateDecryptor(Convert.FromBase64String(CKEY), Convert.FromBase64String(_CIV)); //用指定的密钥和初始化向量创建对称数据解密标准
            }
